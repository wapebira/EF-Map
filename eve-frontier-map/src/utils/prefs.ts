...
export type EFMapPreferences = EFMapPreferencesV1|EFMapPreferencesV2|EFMapPreferencesV3|EFMapPreferencesV4|EFMapPreferencesV5|EFMapPreferencesV6|EFMapPreferencesV7|EFMapPreferencesV8|EFMapPreferencesV9|EFMapPreferencesV10;

const KEY='efmap:prefs';
const defaultPrefsV10: EFMapPreferencesV10 = { v:10, accent:'orange', openPanels:[], uiScale:1, showStations:false, overlaySort:'createdAt:-1', overlayAgingDays:3, transmissionSeen:false, transmissionAudioMuted:false, transmissionAudioVolume:0.25, gateGradientSpan:0.66, hoverPrecisionFloor:0.12, pulseSpeed:1.0, pulseHeadSize:0.25, pulseTailSize:0.65, pulseWidth:0.15, showShipDash:true, pulseBrightness:1.0, starSizeScale:1.0 };

export function loadPrefs(): EFMapPreferences {
  try {
  const raw = localStorage.getItem(KEY); if(!raw) return { ...defaultPrefsV10 };
  const parsed = JSON.parse(raw); if(typeof parsed !== 'object' || parsed === null) return { ...defaultPrefsV10 };
  const upgradeTo8 = (base:any): EFMapPreferencesV8 => ({ ...defaultPrefsV10, ...base, v:8 });
  const upgradeTo9 = (base:any): EFMapPreferencesV9 => ({ ...defaultPrefsV10, ...base, v:9, showShipDash: base.showShipDash !== false });
  const upgradeTo10 = (base:any): EFMapPreferencesV10 => ({ ...defaultPrefsV10, ...base, v:10, pulseBrightness: base.pulseBrightness ?? 1.0, starSizeScale: base.starSizeScale ?? 1.0 });
    switch(parsed.v){
  ...
  case 8:{ return upgradeTo10(upgradeTo9(parsed)); }
  case 9:{ return upgradeTo10(parsed); }
  case 10:{ return { ...defaultPrefsV10, ...parsed }; }
  default: return { ...defaultPrefsV10 };
    }
  } catch { return { ...defaultPrefsV10 }; }
}
...
export function resetAllPrefs(){ currentPrefs={ ...defaultPrefsV10 }; try{ localStorage.removeItem(KEY);}catch{} }

...
export function createRouteRibbon(opts: RouteRibbonOptions): THREE.Group | null {
  ...
  interface Hop { pts: THREE.Vector3[]; length: number; isShip: boolean; }
  const hops: Hop[] = [];
  ...
  hops.push({ pts, length: dist, isShip: false });
      } else {
  hops.push({ pts: [A, B], length: dist, isShip: false });
      }
    } else {
      const dist = A.distanceTo(B);
  ...
  hops.push({ pts: filtered, length: dist, isShip: true });
    }
  }
  ...
  u_dashFade: { value: 0.65 },  // off brightness floor

...
function applyEvent(s, type){
  const def = EVENT_MAP.get(type); if(!def) return false; s.updatedAt=new Date().toISOString(); if(def.counters) def.counters.forEach(k=>{ s.counters[k]=(s.counters[k]||0)+1; }); return true;
}
...
async function handleUsageEvent(req, env){
  ...
  if(known){
    ...
    if(applyEvent(current,type) && applyEvent(daily,type)){

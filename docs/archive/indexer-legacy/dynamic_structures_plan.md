# Archived: Dynamic Player Structures & Smart Gates – Planning Document (Draft)

This is an archived copy of `docs/dynamic_structures_plan.md` preserved for historical reference after the project pivoted to a snapshot-first, KV-first architecture. It reflects the earlier D1/standalone indexer path. Current active approach: KV-backed snapshots with minimal overlays; D1 plans are deferred.

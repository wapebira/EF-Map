...
Scope: Maps D1 schema columns (draft in `d1_schema_draft.sql`) to World API payload fields and anticipated chain (MUD Store) table/event sources. This acts as the contract for the initial migration. Any change after this point requires a migration bump (v2) and decision-log entry.
